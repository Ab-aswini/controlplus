...
export default function BlogDetailPage() {
  ...
  return (
    <article className="max-w-3xl mx-auto px-4 sm:px-6 py-8 md:py-12">
      ...
      <h1 className="text-3xl md:text-4xl font-bold text-gray-900 dark:text-white mb-4 mt-4">{post.title}</h1>
      ...
      {post.cover_image && (
        <img src={post.cover_image} alt={post.title} className="w-full rounded-xl mb-8 max-h-[400px] object-cover" />
      )}
      ...
      <div className="prose prose-gray dark:prose-invert max-w-none">
        {post.content.split('\n').map((paragraph, i) => {
          ...
          if (paragraph.startsWith('- ')) {
            return (
              <div key={i} className="text-gray-600 dark:text-gray-400 ml-4 mb-1 pl-2 relative before:content-['•'] before:absolute before:-left-3 before:text-gray-400">
                {paragraph.replace('- ', '')}
              </div>
            );
          }
